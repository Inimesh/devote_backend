...
# create users
user1 = User.new(username: 'John Smith', email: 'johnsmith@mail.com', password: 'password',
                 password_confirmation: 'password')
user2 = User.new(username: 'Mary Williams', email: 'marywilliams@mail.com', password: 'password',
                 password_confirmation: 'password')
user1.save
user2.save
# add configs
...
users.each do |user|
  5.times do
    transaction = Transaction.new(amount: BigDecimal(Faker::Commerce.price(range: 5.0..200.0, as_string: true)),
                                  user_id: user.id)
    ...
    transaction.name = if transaction.amount <= 35
                         vendors[:small].sample
                       elsif transaction.amount > 35 && transaction.amount <= 110
                         vendors[:medium].sample
                       else
                         vendors[:large].sample
                       end
